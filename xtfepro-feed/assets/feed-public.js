...
(function () {
  ...
  var data = window.xtfeproFeedData || {};
  ...
  function setupModals() {
    modalEventIds.forEach(function (eventId) {
      var btnId = 'xtfeprofeed-ticket-btn-' + eventId;
      ...
      try {
        ...
        btn.addEventListener('click', function () {
          var card = btn.closest('.xtfeprofeed-event-card');
  ...
  function loadPage(feedWrap, feedId, page, perPage) {
    ...
    // Show loading state
    feedWrap.classList.add('xtfeprofeed-loading');
    feedWrap.setAttribute('data-loading', 'true');
    eventsContainer.style.opacity = '0.5';

    // Build form data
    var formData = new FormData();
    formData.append('action', 'xtfeprofeed_load_page');
    ...
    .then(function (result) {
      feedWrap.classList.remove('xtfeprofeed-loading');
  ...
  function showError(container, message) {
    ...
    errorDiv.className = 'xtfeprofeed-feed-error';
  ...
  function initInfiniteScroll(feedWrap) {
    ...
    var sentinel = feedWrap.querySelector('.xtfeprofeed-infinite-sentinel');
    if (!sentinel) return;
    ...
    var observer = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting && !feedWrap.classList.contains('xtfeprofeed-loading')) {
